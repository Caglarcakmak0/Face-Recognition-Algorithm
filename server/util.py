...
def load_saved_artifacts():
    ...
    with open(r"D:\Code\CelebrityFaceRegognition\server\artifacts\class_dictionary.json", "r") as f:
        __class_name_to_number = json.load(f)
        __class_number_to_name = {v:k for k,v in __class_name_to_number.items()}

    global __model
    if __model is None:
        with open(r'D:\Code\CelebrityFaceRegognition\server\artifacts\saved_model.pkl', 'rb') as f:
...
def get_cropped_image_if_2_eyes(image_path, image_base64_data):
    ...
    eye_cascade = cv2.CascadeClassifier(r'D:\Code\CelebrityFaceRegognition\server\opencv\haarcascades\haarcascade_eye.xml')
    if image_path:
...
def get_b64_test_image_for_virat():
    with open(r"D:\Code\CelebrityFaceRegognition\server\b64.txt") as f:
        return f.read()
